...
class TypeActivityController extends Controller
{
    ...
    {
        $url = env('URL_BASE_API', "http://localhost:8000");
        $response = Http::acceptJson()->withToken(Session::put('token'))->post($url . '/type_activity' . $id,[
            'id' => $request->id,
            'description' => $request->description
        ]);
        if ($response->successful()) {
    ...
    {
        $url = env('URL_BASE_API', "http://localhost:8000");
        $response = Http::acceptJson()->withToken(Session::put('token'))->delete($url . '/type_activity' . $id);
        if ($response->successful()) {
